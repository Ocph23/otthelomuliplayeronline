...
function Chessboard() {
	...
	oo.create = function() {
		...
		piecesnum = document.getElementById('side1').getElementsByTagName('span');
		side = {
...
function Othello() {
	var oo = this;
	oo.connection = new signalR.HubConnectionBuilder().withUrl('/gameHub').build();
	oo.connection.start().then(function() {}).catch(function(err) {
	...
	function aiRun() {
		if (map.nextNum == 1) oo.go(map.nextIndex[0]);
		else if (map.space <= 58) oo.go(ai.startSearch(map));
		else oo.go(map.nextIndex[(Math.random() * map.nextIndex.length) >> 0]);
	}
